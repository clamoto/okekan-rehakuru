    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-20">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          <Link
            href="/schedules"
            className="inline-flex items-center gap-1.5 text-slate-600 hover:text-slate-900 text-xs font-semibold transition"
          >
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-200">
            <div>
              ...
              <h1 className="text-2xl font-bold text-slate-900">{mockScheduleDetails.title}</h1>
            </div>

            <div className="flex items-center gap-2">
              <Link
                href={`/stage-layout?schedule_id=${scheduleId}`}
                className="inline-flex items-center gap-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-bold text-xs px-3.5 py-2 rounded-xl transition"
              >
                <Layers className="w-3.5 h-3.5" />
                舞台配置・ひな壇計算
              </Link>
              <button className="inline-flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold px-3.5 py-2 rounded-xl transition border border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 text-xs text-slate-700">
            <div className="flex items-start gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200">
              ...
              <div>
                <span className="font-bold text-slate-900 block mb-0.5">日時・タイムスケジュール</span>
                <p className="text-slate-700">{mockScheduleDetails.dateStr}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200">
              ...
              <div>
                <span className="font-bold text-slate-900 block mb-0.5">会場・アクセス</span>
                <p className="text-slate-700">{mockScheduleDetails.location}</p>
              </div>
            </div>
          </div>

          <div className="mt-4 bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs">
            <span className="font-bold text-slate-900 block mb-1">練習曲目:</span>
            <div className="flex flex-wrap gap-2">
              {mockScheduleDetails.pieces.map((piece, idx) => (
                <span
                  key={idx}
                  className="bg-white border border-slate-200 text-slate-800 px-3 py-1 rounded-lg font-medium"
                >
                  <Music className="w-3 h-3 inline mr-1 text-blue-600" />
                  {piece}
                </span>
              ))}
            </div>
            {mockScheduleDetails.notes && (
              <p className="text-slate-600 text-xs mt-3 pt-2 border-t border-slate-200">
